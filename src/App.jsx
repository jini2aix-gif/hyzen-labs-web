...
// --- [Visualization: Intelligent Fluid Bubble] ---
...
const App = () => {
  ...
  return (
    <div className="h-screen w-screen bg-[#010101] text-white selection:bg-cyan-500/30 overflow-hidden font-sans flex flex-col relative">
      ...
      {/* --- [Entry Sequence] --- */}
      {isInitializing && (
        <div className="fixed inset-0 z-[10000] bg-black flex flex-col items-center justify-center p-8 transition-opacity duration-700 text-white text-center">
      ...
      {/* --- [Structural Backdrop for 3D Protection] --- */}
      {isAnyModalOpen && (
      ...
      >
        ...
        <nav className="w-full z-[100] px-6 py-4 flex justify-between items-center shrink-0 text-white">
            ...
            <div key={activeView} className="w-full h-full animate-fade-in-soft">
              {activeView === 'roadmap' && (
                <CoverFlow items={roadmapSteps} activeIndex={activeIndices.roadmap} setActiveIndex={(i) => setViewIndex('roadmap', i)} renderItem={(step) => (
                  <div onClick={() => { setSelectedItem(step); setIsModalOpen(true); }} className="w-full h-full glass-panel p-6 rounded-[2.5rem] border border-cyan-500/30 flex flex-col justify-between tap-feedback group hover:border-cyan-400 transition-colors shadow-2xl text-white">
              ...
              {activeView === 'traces' && (
                messages.length > 0 ? (
                  <CoverFlow items={messages} activeIndex={activeIndices.traces} setActiveIndex={(i) => setViewIndex('traces', i)} renderItem={(msg) => (
                    <div className="w-full h-full glass-panel rounded-[2.5rem] relative overflow-hidden border border-violet-500/30 group shadow-[0_15px_40px_rgba(0,0,0,0.5)] text-white">
                      {msg.image && (
        ...
        <div className="fixed inset-0 z-[4000] flex items-end sm:items-center justify-center p-0 sm:p-4 animate-fade-in-soft" onClick={closeModal}>
          <div className="w-full h-[80vh] sm:h-auto sm:max-w-xl glass-panel rounded-t-[3.5rem] sm:rounded-[3.5rem] p-10 relative overflow-y-auto no-scrollbar border-t border-white/10 text-white shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="text-left text-white text-left">
              ...
              <h2 className="text-3xl font-black mb-10 uppercase tracking-tighter leading-tight text-white">
                {String(selectedItem.title)}
              </h2>
              <div className="space-y-8 pb-12 text-white text-left">
                <section>
                  <h4 className="text-[10px] font-brand tracking-[0.2em] uppercase font-bold text-white/30 mb-3 border-l-2 border-cyan-500 pl-4">Goal</h4>
                  <p className="text-[15px] font-light text-white/80 leading-relaxed pl-5">{String(selectedItem.goal)}</p>
                </section>
                <section>
                  <h4 className="text-[10px] font-brand tracking-[0.2em] uppercase font-bold text-white/30 mb-3 border-l-2 border-cyan-500 pl-4">Process</h4>
                  <p className="text-[15px] font-light text-white/80 leading-relaxed pl-5">{String(selectedItem.process)}</p>
                </section>
                <section>
                  <h4 className="text-[10px] font-brand tracking-[0.2em] uppercase font-bold text-white/30 mb-3 border-l-2 border-cyan-500 pl-4">Result</h4>
                  <p className="text-[15px] font-light text-white/80 leading-relaxed pl-5">{String(selectedItem.result)}</p>
                </section>
              </div>
              <button onClick={closeModal} className="w-full bg-white text-black py-5 rounded-[2.2rem] font-brand text-[11px] font-black tracking-[0.3em] uppercase shadow-2xl hover:bg-cyan-400 transition-colors">Close Intelligence Hub</button>
            </div>
          </div>
        </div>
      )}
