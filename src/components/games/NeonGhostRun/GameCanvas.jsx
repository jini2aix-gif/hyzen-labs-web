...
// ─── Biomes ──────────────────────────────────────────────────────────────────
const BIOMES = [
    { name: 'NEON CITY', bg: '#06030F', sky1: '#0d0621', sky2: '#120830', grid: '#3b1f6e', accent: '#c026d3', glow: '192,38,211', trail: '#e879f9' },
    { name: 'DATA STREAM', bg: '#030A10', sky1: '#031220', sky2: '#041a2e', grid: '#0c4a6e', accent: '#0ea5e9', glow: '14,165,233', trail: '#7dd3fc' },
    { name: 'VOID SECTOR', bg: '#0F0305', sky1: '#1a0309', sky2: '#220410', grid: '#7f1d1d', accent: '#f43f5e', glow: '244,63,94', trail: '#fb7185' },
    { name: 'HYZEN ZONE', bg: '#030A05', sky1: '#041208', sky2: '#051a0a', grid: '#14532d', accent: '#22c55e', glow: '34,197,94', trail: '#86efac' },
];

// ─── Items ───────────────────────────────────────────────────────────────────
// SHIELD: absorb 1 hit     GHOST: 4s invulnerability     BOOST: 2× combo score
const ITEMS = {
    shield: { color: '#f43f5e', glow: '244,63,94', label: 'SHIELD ♥', emoji: '♥' },
    ghost: { color: '#a78bfa', glow: '167,139,250', label: 'PHANTOM ◈', emoji: '◈' },
    boost: { color: '#fbbf24', glow: '251,191,36', label: 'BOOST ✦', emoji: '✦' },
};
...
// ─── Obstacle / Pattern factory ───────────────────────────────────────────────
// obsBottom (aerial) must be:
//   > standing player.top (gy - PH_STAND + 4 = gy-48) → HIT when standing
//   ≤ sliding  player.top (gy - PH_STAND + (PH_STAND-PH_SLIDE) + 4 = gy-16) → MISS when sliding
// Aerial: y=gy-90, h=50 → bottom=gy-40  ← gy-48 < gy-40 ✓ HIT standing; gy-16 < gy-40? No ✓ MISS sliding
const makeGround = (x, gy, wide = false) => ({ type: 'ground', x, y: gy - 52, w: wide ? 85 : 42, h: 52 });
const makeAerial = (x, gy) => ({ type: 'aerial', x, y: gy - 90, w: 54, h: 50 });
const makeMoving = (x, gy) => ({ type: 'moving', x, y: gy - 110, baseY: gy - 110, w: 30, h: 30, amp: 60, phase: Math.random() * Math.PI * 2 });
const makeTall = (x, gy) => ({ type: 'tall', x, y: gy - 80, w: 36, h: 80 }); // needs double-jump

// Level-gated patterns. Returns array of obstacle objects.
const spawnPattern = (level, x, gy) => {
    const roll = Math.random();
    if (level === 1) {
        return [makeGround(x, gy)];
    }
    if (level === 2) {
        return roll < 0.5 ? [makeGround(x, gy)] : [makeAerial(x, gy)];
    }
    if (level === 3) {
        if (roll < 0.35) return [makeGround(x, gy)];
        if (roll < 0.65) return [makeAerial(x, gy)];
        // combo: jump then slide
        return [makeGround(x, gy), makeAerial(x + 200, gy)];
    }
    if (level === 4) {
        if (roll < 0.25) return [makeGround(x, gy)];
        if (roll < 0.45) return [makeAerial(x, gy)];
        if (roll < 0.65) return [makeAerial(x, gy), makeGround(x + 200, gy)]; // slide then jump
        if (roll < 0.80) return [makeTall(x, gy)];   // tall → double jump needed
        return [makeGround(x, gy), makeAerial(x + 180, gy)];
    }
    // level 5+
    if (roll < 0.20) return [makeGround(x, gy)];
    if (roll < 0.35) return [makeAerial(x, gy)];
    if (roll < 0.50) return [makeTall(x, gy)];
    if (roll < 0.65) return [makeGround(x, gy), makeAerial(x + 190, gy)];
    if (roll < 0.80) return [makeAerial(x, gy), makeGround(x + 190, gy)];
    if (roll < 0.90) return [makeMoving(x, gy)];
    // triple pattern
    return [makeGround(x, gy), makeAerial(x + 200, gy), makeGround(x + 410, gy)];
};

// ─── Draw helpers ─────────────────────────────────────────────────────────────
const drawObstacle = (ctx, o, biome, now) => {
    const { x, y, w, h, type } = o;
    const color = type === 'tall' ? biome.accent : o.type === 'aerial' ? '#0ea5e9' : biome.accent;
    ctx.save();
    setGlow(ctx, color, 18);
    ctx.strokeStyle = color;
    ctx.lineWidth = 2.5;
    ctx.fillStyle = `rgba(${type === 'aerial' ? '14,165,233' : biome.glow},0.13)`;
    ctx.beginPath();
    ctx.roundRect(x, y, w, h, 5);
    ctx.fill(); ctx.stroke();

    // Inner scanline
    ctx.fillStyle = `rgba(${type === 'aerial' ? '14,165,233' : biome.glow},0.4)`;
    ...
    if (type === 'ground' || type === 'tall') {
        ctx.fillText(type === 'tall' ? '▲▲ D-JUMP' : '▲ JUMP', x + w / 2, y - 6);
    } else if (type === 'aerial') {
        ctx.fillText('▼ SLIDE', x + w / 2, y + h + 13);
    }
...
const drawItem = (ctx, item, now) => {
    ...
    ctx.fillText(kind.toUpperCase(), x, y + r + 8);
...
const drawRunner = (ctx, x, y, isSliding, isAir, frame, biome, alpha) => {
    ...
    if (isSliding) {
        ...
        ctx.translate(cx, y + PH_STAND * 0.5);
        ctx.rotate(lean);
        // Body
...
// ─── Stars & Buildings (stable across renders) ───────────────────────────────
...
const PARALLAX = [0.07, 0.13, 0.20];
const ALPHAS = [0.14, 0.22, 0.35];
...
const drawBg = (ctx, W, H, gy, dist, biome) => {
    ...
    const off = dist % 80;
    for (let gx = -off; gx < W + 80; gx += 80) {
        ctx.lineWidth = 0.6; ctx.beginPath(); ctx.moveTo(gx, gy); ctx.lineTo(gx + 40, H * 1.1); ctx.stroke();
    }
    for (let row = 0; row < 5; row++) {
        ctx.lineWidth = 0.4 - row * 0.06; ctx.beginPath();
        ctx.moveTo(0, gy + (H - gy) * (row / 5)); ctx.lineTo(W, gy + (H - gy) * (row / 5)); ctx.stroke();
    }
    ctx.restore();
    // Ground line
...
const GameCanvas = ({ onGameOver, onScoreUpdate, onLevelChange, onComboUpdate, onItemCollect, gameAudio, ghostData }) => {
    ...
    const getDiff = useCallback(() => {
        ...
        if (w < 480) return { speedMul: 0.80, gapBase: 400, itemGap: 900 };
        if (w < 768) return { speedMul: 0.90, gapBase: 360, itemGap: 800 };
        if (w < 1100) return { speedMul: 1.10, gapBase: 300, itemGap: 700 };
        return { speedMul: 1.28, gapBase: 260, itemGap: 650 };
    }, []);
    ...
    useEffect(() => {
        ...
        G.current = {
            ...
            nextSpawnAt: 300, nextItemAt: 600,
            frameCount: 0, startTime: performance.now(),
            ghostData: ghostData || null, ghostFrames: [], ghostFrame: 0,
            scoreBoost: 1, boostTimer: 0, beatPhase: 0,
        };

        let lastT = performance.now();

        const render = (g, p, biome, W, H, gy, now) => {
            const beat = 1 + Math.sin(now / 350) * 0.04;
            drawBg(ctx, W, H, gy, g.distance, biome);

            // Ghost
            if (g.ghostData) {
                const gf = g.ghostData[Math.floor(g.frameCount / 2)];
                if (gf) drawRunner(ctx, p.x - 55, gf.py, gf.isSliding, gf.py < gy - PH_STAND - 2, g.frameCount, biome, 0.3);
            }
            ...
            g.particles.forEach(par => {
                ctx.save(); ctx.globalAlpha = par.life;
                setGlow(ctx, par.color, 8); ctx.fillStyle = par.color;
            ...
            if (p.shield) {
                ctx.save();
                setGlow(ctx, '#f43f5e', 24);
                ctx.strokeStyle = '#f43f5e'; ctx.lineWidth = 2;
                ctx.globalAlpha = 0.6 + Math.sin(now / 200) * 0.2;
                ctx.beginPath(); ctx.arc(p.x + PLAYER_W / 2, p.y + PH_STAND / 2, 32, 0, Math.PI * 2); ctx.stroke();
                clearGlow(ctx); ctx.restore();
            }

            // Float texts
            g.floatTexts.forEach(ft => {
                ctx.save(); ctx.globalAlpha = Math.min(ft.life, 1);
                setGlow(ctx, ft.color, 10); ctx.fillStyle = ft.color;
                ctx.font = `bold ${ft.size || 15}px "Orbitron",monospace`;
                ctx.textAlign = 'left'; ctx.fillText(ft.text, ft.x, ft.y);
                clearGlow(ctx); ctx.restore();
            });
            ...
            if (g.boostTimer > 0) {
                ...
                setGlow(ctx, '#fbbf24', 8); ctx.fillStyle = '#fbbf24';
                ctx.font = '9px monospace'; ctx.textAlign = 'right';
                ctx.fillText(`✦ ×2 BOOST ${(g.boostTimer / 1000).toFixed(1)}s`, W - 16, 52);
                clearGlow(ctx); ctx.restore();
            }
            ...
            if (W < 768) {
                ...
                ctx.fillRect(0, H * 0.5, W * 0.5, H * 0.5); ctx.restore();
                ctx.save(); ctx.globalAlpha = 0.15; ctx.fillStyle = biome.accent;
                ctx.font = '9px monospace'; ctx.textAlign = 'center';
                ctx.fillText('▼ SLIDE', W * 0.25, H - 18);
                ctx.fillText('▲ JUMP', W * 0.75, H - 18);
                ctx.restore();
            }

            // Progress bar
            ctx.fillStyle = 'rgba(0,0,0,0.4)';
            ctx.fillRect(W / 2 - 90, H - 22, 180, 4);
            setGlow(ctx, biome.accent, 5); ctx.fillStyle = biome.accent;
            ctx.fillRect(W / 2 - 90, H - 22, (g.distance % 2000) / 2000 * 180, 4);
            clearGlow(ctx);
            ctx.save(); ctx.globalAlpha = 0.4; ctx.fillStyle = biome.accent;
            ctx.font = '8px "Orbitron",monospace'; ctx.textAlign = 'center';
            ctx.fillText(`LV.${g.level} · ${biome.name} · ${g.speed.toFixed(1)}×`, W / 2, H - 28);
            ctx.restore();
        };
        ...
        const loop = (now) => {
            ...
            g.frameCount++; g.beatPhase = (now / 400) * Math.PI * 2;

            // ── Level ────────────────────────────────────────────────────────────
            const newLv = Math.floor(elapsed / 25) + 1;
            if (newLv > g.level) {
                g.level = newLv; onLevelChange?.(newLv);
            ...
            // ── Player physics ───────────────────────────────────────────────────
            p.vy += GRAVITY; p.y += p.vy * (dt / 16);
            p.animFrame += dt / 16;
            if (p.y >= gy - PH_STAND) {
                p.y = gy - PH_STAND; p.vy = 0; p.onGround = true; p.jumpCount = 0; p.coyoteF = COYOTE_F;
                if (p.jumpBuf > 0) { p.jumpBuf = 0; doJump(); }
            ...
            if (g.distance >= g.nextSpawnAt) {
                const newObs = spawnPattern(g.level, W + 50, gy);
            ...
            if (g.distance >= g.nextItemAt) {
                ...
                const itemX = W + 50;
                const itemY = gy - 85; // reachable by jumping
                g.items.push({ kind, x: itemX, y: itemY, collected: false });
            ...
            g.obstacles.forEach(o => {
                o.x -= g.speed * (dt / 16);
                if (o.type === 'moving') o.y = o.baseY + Math.sin(now / 700 + o.phase) * o.amp;
            });
            g.obstacles = g.obstacles.filter(o => o.x + (o.w || 60) > -60);
            ...
            for (let i = g.obstacles.length - 1; i >= 0; i--) {
                ...
                if (!o.passed && hit && !p.isInvul) {
                    ...
                    if (p.shield) {
                        // Shield absorbs hit
                        p.shield = false; p.isInvul = true; p.invulTimer = 1200;
                        gameAudio?.playSFX('collect');
                        g.floatTexts.push({ x: p.x, y: p.y - 30, text: 'SHIELD BROKEN!', life: 1.5, color: '#f43f5e', size: 14 });
                    } else {
                        // Crash sparks
                ...
                if (!o.passed && o.x + o.w < p.x) {
                    ...
                    if (g.combo > 0 && g.combo % 5 === 0) {
                        g.floatTexts.push({ x: p.x, y: p.y - 20, text: `✕${g.combo} COMBO!`, life: 1.5, color: biome.accent, size: 14 });
                        gameAudio?.playSFX('collect');
                    }
                }
            }
            ...
            g.items.forEach(it => {
                ...
                const dist2 = Math.hypot(it.x - (p.x + PLAYER_W / 2), it.y - (p.y + PH_STAND / 2));
                if (dist2 < 36) {
                    it.collected = true;
                    onItemCollect?.(it.kind);
                    gameAudio?.playSFX('collect');
                    // Apply effect
                    if (it.kind === 'shield') {
                        p.shield = true;
                        g.floatTexts.push({ x: p.x, y: p.y - 25, text: '♥ SHIELD ON!', life: 2, color: '#f43f5e', size: 15 });
                    } else if (it.kind === 'ghost') {
                        p.isInvul = true; p.invulTimer = 4000;
                        g.floatTexts.push({ x: p.x, y: p.y - 25, text: '◈ PHANTOM 4s!', life: 2, color: '#a78bfa', size: 15 });
                    } else if (it.kind === 'boost') {
                        g.scoreBoost = 2; g.boostTimer = 8000;
                        g.floatTexts.push({ x: p.x, y: p.y - 25, text: '✦ ×2 BOOST!', life: 2, color: '#fbbf24', size: 15 });
                    }
                    // Collect particles
                    const def = ITEMS[it.kind];
                    for (let k = 0; k < 10; k++) g.particles.push({
                        x: it.x, y: it.y,
                        vx: (Math.random() - 0.5) * 8, vy: (Math.random() - 0.5) * 8,
                        life: 0.9, color: def.color, size: 3,
                    });
                }
            });
            ...
            if (g.frameCount % 4 === 0) g.particles.push({
                x: p.x + PLAYER_W / 2, y: p.y + (p.isSliding ? PH_SLIDE : PH_STAND) - 4,
                vx: -g.speed * 0.4 - Math.random(), vy: (Math.random() - 0.5) * 1.5,
                life: 0.65, color: biome.trail, size: 2.5,
            });
